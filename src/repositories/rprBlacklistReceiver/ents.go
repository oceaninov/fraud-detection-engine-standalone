...
type EntityBlacklistReceiverApproval struct {
	Id               string  `gorm:"column:id"`
	PhoneNumber      string  `gorm:"column:phone_number"`
...
func (ent *EntityBlacklistReceiverApproval) TableName() string {
	return "black_list_receiver_approval"
}

func (ent *EntityBlacklistReceiverApproval) DefaultColumns() string {
	return strings.Join([]string{
		fmt.Sprintf("%s.id", ent.TableName()),
		fmt.Sprintf("%s.phone_number", ent.TableName()),
...
func (ent *EntityBlacklistReceiverApproval) ToMap() map[string]interface{} {
	return entityExtractor.ConvertEntityToMap(ent)
}
